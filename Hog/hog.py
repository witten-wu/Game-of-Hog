...
def roll_dice(current_score, num_rolls, dice=six_sided):
    ...
    while num_rolls >= 1:
        cur = dice()
        if   cur == 1 and current_score % 2 == 1:
             flag = 1
             finalsum += 1
             num_rolls -= 1
        elif cur == 1 and current_score % 2 != 1:
             flag = 1
             num_rolls -= 1
        elif current_score % 2 == cur % 2 :
              finalsum += 1
              sum += cur
              num_rolls -=1
        else:
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        if who == 0:
           next = next_player(who)
           point1 = take_turn(strategy0(score0, score1), score0, score1, dice, goal)
           score0 += point1
           if swine_swap(score0, score1) == True:
               score0, score1 = score1, score0
        else:
           next = next_player(who)
           point2 = take_turn(strategy1(score1, score0), score1, score0, dice, goal)
           score1 += point2
           if swine_swap(score1, score0) == True:
    ...
    def announcement(score0, score1):
        ...
        if score > last_score:
            ...
            if  running > running_high:
                runninghigh = running
                if who == 0:
                  print(runninghigh , 'point(s)!','That\'s a record gain for Player 0!')
                else:
                  print(runninghigh , 'point(s)!','That\'s a record gain for Player 1!')  
        return announce_highest(who, score, runninghigh)
    return announcement
...
def max_scoring_num_rolls(current_score=0, dice=six_sided, trials_count=1000):
    ...
    max_roll = 0
    for i in range(1,11):
        max_roll_i = make_averaged(roll_dice, trials_count)(current_score, i, dice)
        if max_roll_i > max_roll:
            max_roll = max_roll_i
            cout = i
    return cout
